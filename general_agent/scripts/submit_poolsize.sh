#!/bin/bash
# Pool-size ablation: vary --pool-size for div_k4 on qwen3-1.7b + qwen3-8b.
# Fixed: k=4, T=8, max_tok=2048, datasets = {hotpotqa, GAIA}, pools = {4,8,16,32}, seeds = 1..3.
#
# Usage: ./scripts/submit_poolsize.sh
#
# Writes to: /home/ssmurali/t3-testbed/results/poolsize_ablation/<model>/<dataset>/pool_<P>/run_<seed>/

set -euo pipefail
cd "$(dirname "$0")/.."
mkdir -p logs/poolsize logs/judge
...
RESULTS_BASE="/home/ssmurali/t3-testbed/results/poolsize_ablation"
...
for MODEL_SHORT in "${MODELS[@]}"; do
  ...
  for DATASET in "${DATASETS[@]}"; do
    ...
    for POOL in "${POOLS[@]}"; do
      OUTDIR_BASE="${RESULTS_BASE}/${MODEL_SHORT}/${DATASET}/pool_${POOL}"
      ...
      for SEED in "${SEEDS[@]}"; do
        ...
        INNER+="; echo '--- ${MODEL_SHORT} | ${DATASET} | pool=${POOL} | seed=${SEED} ---'"
        INNER+="; python -m webwalkerqa.run.run_main_table --model ${LITELLM_MODEL} --dataset ${DATASET_PATH} --condition diversity_parallel --seed ${SEED} --max-turns-par 8 --k 4 --max-tokens 2048 --pool-size ${POOL} --max-concurrent 100 --api-base http://localhost:\$VLLM_PORT/v1 --output-dir ${OUTDIR} || echo '[warn] seed ${SEED} exited nonzero'"
      done
      INNER+="; echo 'Done seeds: ${MODEL_SHORT}/${DATASET}/pool_${POOL}'"
      INNER+="; echo '--- judging ${MODEL_SHORT}/${DATASET}/pool_${POOL} ---'"
      INNER+="; python -m webwalkerqa.judge.eval_llm --results-dir ${RESULTS_BASE} --filter-model ${MODEL_SHORT} --filter-dataset ${DATASET} --filter-condition pool_${POOL} --model openai/gpt-4o-mini --max-concurrent 200 --force || echo '[warn] judge failed'"
      INNER+="; kill \$VLLM_PID 2>/dev/null || true; wait \$VLLM_PID 2>/dev/null || true"

      JNAME="pa-${MODEL_SHORT:0:6}-${DATASET:0:6}-p${POOL}"
      EXP_JOB_ID=$(sbatch \
        --job-name="${JNAME}" --partition=general --gres=gpu:1 \
        --time=24:00:00 --mem=32G --cpus-per-task=8 --export=ALL \
        --output="logs/poolsize/${MODEL_SHORT}_${DATASET}_pool${POOL}_%j.out" \
        --error="logs/poolsize/${MODEL_SHORT}_${DATASET}_pool${POOL}_%j.err" \
